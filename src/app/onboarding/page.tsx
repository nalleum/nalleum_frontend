    ...
    <AppShell className="page-enter bg-[radial-gradient(circle_at_20%_15%,#eef2ff_0%,#f8ebff_45%,#eaf2ff_100%)]">
      <div className="space-y-8 pt-4">
        <div className="space-y-2">
          <p className="text-[14px] font-bold text-muted">맞춤 정보 설정</p>
          <h1 className="text-[48px] font-black leading-[1.08] tracking-[-0.03em] text-text">
            회원님에게 꼭 맞는
            <br />
            공고와 이슈를 추천할게요.
          </h1>
          <p className="text-[20px] font-semibold text-[#6e7fa9]">관심 키워드를 선택하면 맞춤 홈을 구성합니다.</p>
        </div>

        <section className="space-y-5 rounded-[28px] border border-line bg-white p-5 shadow-[var(--shadow-card)]">
          <div>
            <p className="text-[14px] font-bold text-muted">닉네임</p>
            <input
              value={nickname}
              onChange={(event) => setNickname(event.target.value)}
              placeholder="이름 또는 닉네임"
              className="mt-2 w-full rounded-2xl border border-line px-4 py-3 text-[17px] font-semibold text-text"
            />
          </div>
          ...
          <div>
            <div className="flex items-center justify-between">
              ...
              <span className="text-[12px] font-extrabold text-primary">최대 5개</span>
            </div>
          ...
          <div>
            <p className="text-[14px] font-bold text-muted">관심 직무 (필수)</p>
            <div className="mt-2 flex flex-wrap gap-2">
          ...
          <div>
            <p className="text-[14px] font-bold text-muted">관심 키워드</p>
            <input
              value={keywords}
          ...
          <div>
            <p className="text-[14px] font-bold text-muted">알림 시간</p>
            <input
              type="time"
              value={pushTime}
          ...
          <button
            ...
            className="w-full rounded-2xl bg-navy py-4 text-[26px] font-black text-white disabled:opacity-45"
          >
            설정 완료
          </button>
        </section>
