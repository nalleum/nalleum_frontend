  ...
  return (
    <AppShell className="page-enter">
      <div className="space-y-6 pb-20">
        <header className="flex items-center justify-between">
          <button
            type="button"
            onClick={() => router.back()}
            className="rounded-xl border border-line bg-white px-3 py-2 text-[11px] font-bold text-muted"
          >
            ← 뒤로
          </button>
          <p className="text-[15px] font-extrabold text-text">이슈 상세</p>
          <button
            type="button"
        ...
        <section className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] font-bold text-muted">{insight.date}</span>
          </div>
          <h1 className="text-[38px] font-black leading-[1.12] tracking-[-0.03em] text-text">{insight.title}</h1>
        ...
        <section className="space-y-3 rounded-[28px] border border-line bg-white p-5 shadow-[var(--shadow-card)]">
          <h2 className="text-[26px] font-black text-text">핵심 요약</h2>
          <div className="space-y-3 rounded-2xl bg-[#f8faff] p-4">
            {insight.summaryBullets.map((bullet, index) => (
              <div key={bullet} className="flex gap-3">
                <span className="mt-0.5 inline-flex h-6 w-6 items-center justify-center rounded-full bg-primary text-[11px] font-extrabold text-white">
                  {index + 1}
                </span>
                <p className="text-[13px] font-semibold leading-relaxed text-[#33456c]">{bullet}</p>
              </div>
            ))}
          </div>
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex w-full items-center justify-between rounded-2xl border border-line bg-white px-4 py-4 text-[12px] font-bold text-[#3c4d74]"
          >
        ...
        <section className="space-y-3 rounded-[28px] border border-line bg-white p-5 shadow-[var(--shadow-card)]">
          <div className="flex items-center justify-between">
            <h2 className="text-[26px] font-black text-text">[{profile.targetRole}] 면접 예상 질문</h2>
            <span className="rounded-full bg-primary-soft px-3 py-1 text-[11px] font-extrabold text-primary">1 Question</span>
          </div>
          <div className="rounded-2xl bg-[#f8faff] p-4">
            <p className="text-[21px] font-black text-primary">Q1.</p>
            <p className="mt-2 text-[23px] font-black leading-[1.2] tracking-[-0.02em] text-text">{qa.question}</p>
            <div className="mt-4 rounded-xl border border-line bg-white p-3">
              <p className="text-[11px] font-extrabold text-primary">IDEAL ANSWER GUIDE</p>
              <p className="mt-2 text-[12px] font-semibold leading-relaxed text-[#3c4d74]">{qa.answerTip}</p>
            </div>
          </div>
        </section>

        <section className="space-y-3">
          <h2 className="text-[26px] font-black text-text">관련 이슈 더보기</h2>
      ...
      <button
        type="button"
        onClick={() => toggleScrap(insight.id)}
        className={`fixed bottom-6 right-6 z-30 rounded-full px-6 py-4 text-[12px] font-black shadow-[0_12px_24px_rgba(79,45,255,0.25)] ${
