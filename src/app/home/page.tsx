      ...
      <div className="space-y-8">
        <header className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            />
            <div>
              <p className="text-[12px] font-semibold text-muted">반가워요!</p>
              <p className="text-[26px] font-extrabold text-text">{profile.nickname || "회원"}님</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <section className="rounded-[30px] bg-gradient-to-br from-primary to-[#6d55ff] p-6 text-white shadow-[0_18px_30px_rgba(79,45,255,0.25)]">
          ...
          <p className="mt-3 text-[18px] font-bold">{hero.companyName} 하반기 공채</p>
          <div className="mt-1 flex items-end gap-2">
            <p className="text-[56px] font-black leading-none">D-12</p>
            <p className="pb-2 text-[20px] font-bold">최종 면접까지</p>
          </div>
          <p className="mt-5 text-[14px] font-semibold opacity-95">반갑습니다, {profile.targetRole} 개발자님</p>
        </section>

        <section className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-[30px] font-black tracking-[-0.02em] text-text">IT/서비스 업계 HOT 이슈</h2>
            <span className="text-[13px] font-semibold text-muted">전체보기</span>
          </div>
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-[30px] font-black tracking-[-0.02em] text-text">관심 기업 최신 동향</h2>
            <span className="text-[13px] font-semibold text-muted">{profile.targetCompanies.length}개 기업</span>
          </div>
          ...
          {hero ? (
            <article className="rounded-[30px] border border-line bg-card p-4 shadow-[var(--shadow-card)]">
              <div className="flex items-center gap-2">
                ...
                />
                <span className="text-[18px] font-bold text-text">{hero.companyName}</span>
              </div>
              ...
              <div className="mt-4 rounded-2xl bg-[#f9fbff] p-4">
                <p className="text-[13px] font-extrabold text-primary">AI 요약</p>
                <p className="mt-2 text-[16px] font-semibold leading-relaxed text-[#33456c]">{hero.summary}</p>
              </div>
              <button
                type="button"
                onClick={() => router.push(`/detail/${hero.id}`)}
                className="mt-4 w-full rounded-full border border-line bg-white py-3 text-[17px] font-black text-primary"
              >
                관련 예상 질문 확인하기
              </button>
            </article>
