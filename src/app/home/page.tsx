  ...
  const filtered = useMemo(
    () =>
      insights.filter((item) =>
        profile.targetCompanies.length ? profile.targetCompanies.includes(item.companyName) : true
      ),
      ...
      <header className='flex items-center justify-between border-b border-line pb-4'>
        <div className='flex items-center gap-3'>
          <ProfileAvatar
            ...
            size={46}
            className='h-11 w-11 rounded-full border-2 border-primary-soft'
          />
          <div>
        ...
        <div className='flex items-center gap-2'>
          <button
            ...
            className='relative rounded-full p-2 text-[#7f8fb1]'
          >
            <Bell size={22} />
            <span className='absolute right-1.5 top-1.5 h-2 w-2 rounded-full bg-red-500' />
          </button>
          <button type='button' className='rounded-full p-2 text-[#7f8fb1]'>
            <Menu size={22} />
          </button>
        </div>
      </header>
      ...
      <main className='space-y-7 pt-6 pb-8'>
        <section className='rounded-3xl bg-gradient-to-br from-[#6a37ff] to-[#4e2cf7] p-5 text-white shadow-[0_18px_34px_rgba(90,53,255,0.35)]'>
          ...
          <div className='mt-4 flex items-center justify-between rounded-xl bg-black/10 px-3 py-2'>
            <div className='flex -space-x-1.5'>
              ...
              <span className='inline-flex h-6 w-6 items-center justify-center rounded-full border-2 border-[#6f4dff] bg-[#8e74ff] text-[8px] font-bold'>
                +12
              </span>
            </div>
            <p className='text-[11px]'>
              동일 직무 <strong>42명</strong> 준비 중
            </p>
          </div>
        </section>
        ...
        <section className='space-y-3'>
          <div className='flex items-center justify-between'>
            <h3 className='flex items-center gap-2 text-[22px] font-black text-text'>
          ...
          {filtered.slice(0, 2).map((item) => (
            <button
              ...
              className='w-full rounded-2xl border border-line bg-white p-4 text-left shadow-[var(--shadow-card)]'
            >
          ...
          {hero ? (
            ...
            >
              ...
              <div className='overflow-hidden rounded-2xl'>
                ...
                <img
                  src={hero.coverImageUrl ?? imageFallback}
                  alt={hero.title}
